# 魔塔API 手写日记图片转文字脚本
基于魔塔（ModelScope）OpenAI兼容接口实现的批量手写日记图片转文字工具，支持多图片批量识别、严格按手写排版转录，适配Qwen3-VL-8B-Instruct等多模态大模型。

## 功能特点
✅ 批量识别：自定义每批识别图片数量（默认3张/批），高效处理大量日记图片  
...
IMAGE_FOLDER = r"图片/"   # 你的日记照片文件夹路径（绝对/相对路径均可）
...
### 2. 准备图片
将需要识别的手写日记图片放入配置的 `IMAGE_FOLDER` 文件夹中，支持的格式：  
